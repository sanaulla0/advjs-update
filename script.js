...
const runDetails = (item)=>{
  ...
  const elements =[
    ...
    {
      label : "Lat : ",
      data  : item.features[0].geometry.coordinates[1],
    },
    {
      label : "Long : ",
      data  : item.features[0].geometry.coordinates[0],
    },
    {
      label : "Offset STD : ",
...
const renderWhather2=async(lat,long)=>{
  ...
  const inputDetails = (item)=>{
    ...
    //   }
       ...
       if(text.value==item.features[0].properties.lat || text.value==item.features[0].properties.lon || text.value==item.features[0].properties.city || text.value==item.features[0].properties.country || text.value==item.features[0].properties.postcode ){
